...
describe("ranColors", () => {
  it("removes the first item from the array and shuffles the remaining items", () => {
    ...
    const colors1 = ["purple", "blue", "green", "yellow", "pink"]
    // Expected output example (can be a different order): ["yellow", "blue", "pink", "green"]
    expect(ranColors(colors1)).toEqual(expect.not.arrayContaining(["purple"]))

    const colors2 = ["chartreuse", "indigo", "periwinkle", "ochre", "aquamarine", "saffron"]
    expect(ranColors(colors2)).toEqual(expect.not.arrayContaining(["chartreuse"]))
  })
})
...
function combinedArray(...arrays) {
  ...
  // console.log("new Set", new Set(mergedArray));
  return uniqueArray;
}
